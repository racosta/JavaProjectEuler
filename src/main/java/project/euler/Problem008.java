 ...
 */
@Slf4j
public class Problem008 implements ProjectEulerProblem {
   public Problem008() {
   }
   ...
   public Object execute() {
      ...
      long prod = max13Prod(intArray, 0);
      log.info(String.format("Problem 8: %d", prod));
    ...
    */
   private static long max13Prod(int[] numbers, int startPos) {
      if (startPos > numbers.length - 13) {
         return 0;
      }

      return Math.max(max13Prod(numbers, startPos + 1), digit13Prod(numbers, startPos));
   }
    ...
    */
   private static long digit13Prod(int[] numbers, int startPos) {
